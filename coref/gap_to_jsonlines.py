...
def tokenize(dataset, vocab_file):
    ...
    for datum in dataset:
        ...
        if a_end>=len(char_to_word_offset): a_end=-1
        if b_end>=len(char_to_word_offset): b_end=-1
        if pronoun_end>=len(char_to_word_offset): pronoun_end=-1
        ...
        if datum['A-coref'] == 'TRUE' or datum['B-coref'] == 'TRUE':
            entity_span = datum['a_subtoken_span'] if datum['A-coref']=='TRUE' else datum['b_subtoken_span'] # big bug here: datum['A-coref']=='TRUE'
